...
to `ha sling` so users can be warned when picking an experimental runtime.
...
accepted by `ha sling --runtime`. Examples: `"claude"`, `"codex"`, `"pi"`,
`"copilot"`.
...
| Pi | `.claude/CLAUDE.md` | `.pi/extensions/haru-guard.ts` TypeScript extension | Generated by `pi-guards.ts` |
| Copilot | `.github/copilot-instructions.md` | None | No hook mechanism |
| Cursor | `.cursor/rules/haru.md` | None | No hook mechanism |
...
The returned map is merged with `HARU_AGENT_NAME` and
`HARU_WORKTREE_PATH` before being passed to the tmux session creator.
...
delay (see issue haru-3271). Pi's TUI does not have this problem, but its
...
- No `--permission-mode` flag — security is enforced via guard extensions in
  `.pi/extensions/haru-guard.ts`, generated by `src/runtimes/pi-guards.ts`
...
**Key characteristics:**
- `id = "cursor"`, `instructionPath = ".cursor/rules/haru.md"`
...
exercises haru's headless code paths (no tmux, NDJSON over stdout, RPC
...
1. Explicit `name` argument (e.g. from `ha sling --runtime codex`)
...
// Explicit override (from ha sling --runtime flag)
...
ha sling <task-id> --capability builder --name my-agent --runtime myruntime
...
- Mail delivery uses hooks or tmux send-keys; Claude Code uses the
  `UserPromptSubmit` hook (`ha mail check --inject`).
...
```bash
ha sling task-001 --capability builder --name claude-builder --runtime claude
ha sling task-002 --capability builder --name codex-builder --runtime codex
ha sling task-003 --capability scout   --name pi-scout       --runtime pi
```
...
```typescript
// Claude Code native team tools — agents must use ha sling instead
NATIVE_TEAM_TOOLS = [
  "Task", "TeamCreate", "TeamDelete", "SendMessage",
  "TaskCreate", "TaskUpdate", "TaskList", "TaskGet", "TaskOutput", "TaskStop",
]

// Tools requiring human interaction — agents must escalate via ha mail
...
Guard purposes:
- **Block native team tools:** All agents must use `ha sling` for delegation.
...
1. **Path boundary guards** (all agents): Write, Edit, NotebookEdit tools check
   `HARU_WORKTREE_PATH` against the file path extracted from the JSON input.
...
All hooks include an `ENV_GUARD` prefix (`[ -z "$HARU_AGENT_NAME" ] && exit 0`)
...
`.pi/extensions/haru-guard.ts`, generated by `generatePiGuardExtension()`.
...
- `tool_call`: fire-and-forget `ha log tool-start`
- `tool_execution_end`: fire-and-forget `ha log tool-end`
- `agent_end`: awaited `ha log session-end` (task completed normally)
- `session_shutdown`: awaited `ha log session-end` (safety net for crashes/Ctrl+C)
...
runtime:
  ...
  # or when --runtime pi is passed to ha sling).
  pi:
...
```bash
ha sling task-001 --capability builder --name my-builder --runtime codex
```
...
|--------|---------|----------|
| `ha sling --runtime <name>` | `--runtime codex` | Highest |

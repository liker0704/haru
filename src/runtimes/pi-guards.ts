...
// Generates self-contained TypeScript code for .pi/extensions/haru-guard.ts.
//
...
//
// Activity tracking fires via pi.exec("ha log ...") on tool_call,
 ...
 *
 * The returned string is ready to write as `.pi/extensions/haru-guard.ts`.
 ...
 * 1. Block NATIVE_TEAM_TOOLS (all agents) — use ha sling for delegation.
 ...
 * 2. Block INTERACTIVE_TOOLS (all agents) — escalate via ha mail instead.
 ...
 *
 * Activity tracking:
 * - tool_call handler: fire-and-forget "ha log tool-start" to update lastActivity.
 * - tool_execution_end handler: fire-and-forget "ha log tool-end".
 * - agent_end handler: awaited "ha log session-end" — fires when the agentic loop
 *   completes (task done). Without this, completed Pi agents get watchdog-escalated
 *   through stalled → nudge → triage → terminate.
 * - session_shutdown handler: awaited "ha log session-end" — fires on Ctrl+C/SIGTERM.
...
export function generatePiGuardExtension(hooks: HooksDef): string {
	...
	const lines = [
		...
		`// Generated by haru — do not edit manually.`,
		...
		`// pi.exec("ha", [...]) calls the haru CLI for activity tracking and lifecycle.`,
		`import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";`,
		``,
		`const AGENT_NAME = "${agentName}";`,
		`const WORKTREE_PATH = "${worktreePath}";`,
		``,
		`// Native team/task tools blocked (all agents) — use ha sling for delegation.`,
		`// Safety net: Pi does not use Claude Code team tool names natively.`,
		`const TEAM_BLOCKED = ${teamBlockedCode};`,
		``,
		`// Interactive tools blocked (all agents) — escalate via ha mail instead.`,
		...
		`\t * Fire-and-forgets "ha log tool-start" to update lastActivity in the SessionStore,`,
		...
		`\t\tpi.exec("ha", ["log", "tool-start", "--agent", AGENT_NAME, "--tool-name", event.toolName]).catch(() => {});`,
		``,
		`\t\t// 1. Block native team/task tools (all agents).`,
		`\t\tif (TEAM_BLOCKED.has(event.toolName)) {`,
		`\t\t\treturn {`,
		`\t\t\t\tblock: true,`,
		`\t\t\t\treason: \`Overstory agents must use 'ha sling' for delegation — \${event.toolName} is not allowed\`,`,
		`\t\t\t};`,
		`\t\t}`,
		``,
		...
		`\t\t\treturn {`,
		`\t\t\t\tblock: true,`,
		`\t\t\t\treason: \`\${event.toolName} requires human interaction — use ha mail (--type question) to escalate\`,`,
		`\t\t\t};`,
		`\t\t}`,
		``,
		...(isNonImpl
		...
		`\t\t\t\treturn {`,
		`\t\t\t\t\tblock: true,`,
		`\t\t\t\t\treason: "git push is blocked — use ha merge to integrate changes, push manually when ready",`,
		`\t\t\t\t};`,
		`\t\t\t}`,
		...
		`\t\t\t\tif (!branch.startsWith(\`haru/\${AGENT_NAME}/\`)) {`,
		`\t\t\t\t\treturn {`,
		`\t\t\t\t\t\tblock: true,`,
		`\t\t\t\t\t\treason: \`Branch must follow haru/\${AGENT_NAME}/{task-id} convention\`,`,
		`\t\t\t\t\t};`,
		`\t\t\t\t}`,
		`\t\t\t}`,
		capabilityBashBlock,
		...
		``,
		`\t/**`,
		`\t * Tool execution end: fire-and-forget "ha log tool-end" for event tracking.`,
		`\t * Paired with tool_call's tool-start fire for proper begin/end event logging.`,
		`\t */`,
		`\tpi.on("tool_execution_end", async (event) => {`,
		`\t\tpi.exec("ha", ["log", "tool-end", "--agent", AGENT_NAME, "--tool-name", event.toolName]).catch(() => {});`,
		`\t});`,
		``,
		`\t/**`,
		...
		`\t\tawait pi.exec("ha", ["log", "session-end", "--agent", AGENT_NAME]).catch(() => {});`,
		`\t});`,
		``,
		`\t/**`,

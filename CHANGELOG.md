...
- **`ha discover` now coordinator-driven** — replaced direct scout spawning with a coordinator session that autonomously spawns leads and scouts per category, synthesizes results, and writes mulch records
- **`startCoordinatorSession()` extracted** from `coordinator.ts` — reusable core for commands that need coordinator-like sessions with custom names or beacons (used by `ha discover`)
- **New flags:** `--attach` / `--no-attach` and `--watchdog` on `ha discover`; default agent name changed from `discover` to `discover-coordinator`
- **`buildDiscoveryBeacon()` and `buildScoutArgs()` helpers** exported from `discover.ts` for testability

### Fixed

- **Task-lock collision in `ha discover`** — each scout now gets a unique task ID (`taskId-categoryName`) instead of sharing one, preventing `checkTaskLock()` from blocking scouts 2–6
- **`maxAgentsPerLead` rejection in `ha discover`** — scout count now passed via `--max-agents` to avoid exceeding the default cap of 5 when all 6 categories are active
...
- **`ha discover`** — new top-level command that spawns parallel scout agents to explore a brownfield codebase and produce structured mulch records, with categories for architecture, conventions, testing, dependencies, and more
...
#### Profile System
- **`--profile` flag on `ha sling` and `ha coordinator start`** — pass a named profile to customize agent behavior via canopy prompt overlays
...
- CLI command count: 35 → 36 (new `ha discover` command)
...
[Unreleased]: https://github.com/jayminwest/haru/compare/v0.9.1...HEAD
[0.9.1]: https://github.com/jayminwest/haru/compare/v0.9.0...v0.9.1
[0.9.0]: https://github.com/jayminwest/haru/compare/v0.8.7...v0.9.0

/**
 * CLI command: ha hooks install|uninstall|status
 *
 * Manages orchestrator hooks in .claude/settings.local.json.
 * Hooks are sourced from .overstory/hooks.json (generated by ha init).
 *
 ...
 * Running `ha init` alone does NOT modify .claude/ — the user must
 * run `ha hooks install` as a separate step.
 */
...
async function statusHooks(json: boolean): Promise<void> {
	...
	if (json) {
		...
		if (!installed && sourceExists) {
			printHint("Run ha hooks install to install");
		}
	}
}
...
/**
 * Entry point for `ha hooks <subcommand>`.
 */

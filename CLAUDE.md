...
Agents that dispatch sub-agents set `state=waiting` before stopping (`ha status set --state waiting`). The system treats waiting agents specially:
...
Run `ha <command> --help` for full options. Global flags: `--json`, `--verbose`, `--quiet`, `--timing`, `--project <path>`.

### Bootstrap & Setup
```
ha init                   Initialize .overstory/ and bootstrap ecosystem tools
ha quickstart             Guided first-run wizard for new users
ha hooks install|uninstall|status   Manage orchestrator hooks
ha update                 Refresh managed files from installed package
ha upgrade                Upgrade haru (--all for all ecosystem tools)
ha ecosystem              Show os-eco tool versions and health
ha completions            Shell completions
ha config list|get|set|reset   View and modify configuration
ha context generate|show|invalidate   Manage project context cache
```

### Agent Lifecycle
```
ha sling <task-id>        Spawn a worker agent (--capability, --name, --files, --runtime)
ha stop <agent>           Terminate a running agent
ha attach [agent]         Attach to agent's tmux session
ha resume [agent]         Resume interrupted sessions
ha snapshot               Capture swarm state for recovery
ha recover                Restore from snapshot bundle
ha worktree list|clean    Manage git worktrees
ha agents                 Discover available agent definitions
```

### Persistent Orchestration
```
ha coordinator start|stop|status|send|ask|output|check-complete
ha supervisor start|stop|status   [DEPRECATED] Per-project supervisor (use ha sling --capability lead)
ha mission start|stop|pause|resume|complete|answer|list|show|status|output|graph|update|handoff|refresh-briefs|bundle|extract-learnings|artifacts|tier set|tier show|holdout|workstream-complete
ha discover               Brownfield codebase discovery via scout swarm
ha monitor start|stop|status   Tier 2 continuous fleet patrol
ha research start|stop|status|list|output   Deep research sessions
```

### Messaging & Control
```
ha mail send|check|list|read|reply|purge|dlq|retry
ha nudge <agent>          Send text nudge via tmux
...
### Delivery
```
ha merge                  Merge agent branches (--branch, --all, --into, --dry-run)
ha compat check <branch>  Check branch compatibility against canonical
```

### Observability
```
ha status                 Active agents, worktrees, state
ha dashboard              Live TUI dashboard (--interval, --all)
ha inspect <agent>        Deep agent inspection (--follow)
ha trace <target>         Chronological event timeline
ha replay                 Interleaved multi-agent replay
ha feed                   Unified real-time event stream (--follow)
ha logs                   Query NDJSON logs (--agent, --level, --follow)
ha errors                 Aggregated error view
ha costs                  Token/cost analysis (--live, --self, --by-capability)
ha rate-limits            Rate-limit headroom (--live, --runtime)
ha metrics                Session metrics
ha adaptive               Adaptive parallelism state and scaling
ha export status|flush|test   Observability export pipeline
ha webserver start|stop|status|register|discover   HTTP webserver
```
...
ha health-policy          Health policy rules (--execute, history, enable/disable)
ha next-improvement       Top recommendation from health scoring
ha doctor                 Health checks (--fix, --category)
ha eval run|show|list|compare   Scenario-based evaluation

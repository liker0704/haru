/**
 * Tests for `haru inspect` command.
 *
...
describe("inspectCommand", () => {
	...
	describe("validation", () => {
		...
		test("throws if --interval is invalid", async () => {
			...
			store.upsert({
				...
				runtime: "claude",
				worktreePath: "/tmp/wt",
				branchName: "haru/builder-1/test",
				taskId: "haru-001",
	...
	describe("gatherInspectData", () => {
		test("gathers basic session data", async () => {
			...
			expect(data.session.taskId).toBe("haru-001");
	...
	describe("headless agent support", () => {
		...
		test("gatherInspectData provides event-based output for headless agents with tool calls", async () => {
			...
			store.upsert({
				...
				runtime: "claude",
				worktreePath: "/tmp/wt",
				branchName: "haru/headless/task-2",
				taskId: "haru-h02",
				tmuxSession: "", // headless
				state: "working",
				pid: process.pid,
		...
		test("printInspectData shows PID instead of tmux session for headless agents", () => {
			const data = {
				session: {
					...
					runtime: "claude",
					worktreePath: "/tmp/wt",
					branchName: "haru/headless/task-3",
					taskId: "haru-h03",
					tmuxSession: "", // headless
		...
		test("printInspectData shows Recent Activity header for headless agents with tmuxOutput", () => {
			const data = {
				session: {
					...
					runtime: "claude",
					worktreePath: "/tmp/wt",
					branchName: "haru/headless/task-4",
					taskId: "haru-h04",
					tmuxSession: "", // headless
					state: "working" as const,
					pid: 99998,
	...
	describe("stdout.log fallback", () => {
		...
		async function setupHeadlessSession(
			...
			store.upsert({
				...
				branchName: `haru/headless/${agentName}`,
				taskId: "haru-h10",
				tmuxSession: "", // headless
				state: "working",
				pid: process.pid,
	...
	describe("human-readable output", () => {
		test("displays agent metadata", async () => {
			...
			expect(out).toContain("haru-001");
			expect(out).toContain("builder");
			expect(out).toContain("haru/builder-1/test");

...
**Tracking issue:** GitHub `liker0704/haru#189`

This mission rebranded the os-eco tool suite in-place:

- `haru` → `haru` (applied in-place during the mission; operator runs the final disk + GitHub rename)
- `kura` → `kura` (operator runs `scripts/kura.sh`)
- `suji` → `suji` (operator runs `scripts/suji.sh`)
- `tane` → `tane` (operator runs `scripts/tane.sh`)

**Key v3 redesign (D20 — operator-runs-script model):** The original design assumed mission-context leads could run bash directly inside the target repos. This was technically infeasible — agent hooks fire in any terminal with `OVERSTORY_AGENT_NAME` set, making direct shell execution inside a live haru session unsafe. The corrected model: the operator runs the pre-built rebrand scripts from a non-agent terminal after the mission completes.
...
ha watch stop || true                             # tier-0 watchdog
ha status                                         # confirm no agents running

# 2b. No cron / schedules
crontab -l 2>/dev/null | grep -E "(ov |haru)" || echo "OK: no cron"

# 2c. Each sibling repo state check
for d in kura suji tane; do
  echo "=== $d ==="
...
# sister-repo's live origin URL; lead-tane-to-tane confirmed (mission_finding
# msg-ir0rzlct7z74 / msg-u8f9ii1v0yoy) that the rebrand-to-tane branch was published
# to liker0704/tane.git. kura and suji may have produced the same side-effect.
...
for pair in 'tane:rebrand-to-tane' 'kura:rebrand-to-kura' 'suji:rebrand-to-suji'; do
...
for pair in 'kura:rebrand-to-kura' 'suji:rebrand-to-suji' 'tane:rebrand-to-tane'; do
  d="${pair%:*}"; b="${pair#*:}"
  cd /home/liker2/projects/os-eco/$d
...
Each script invocation is followed by `gh repo rename` per operator constraint 5. Haru's disk + GitHub rename happens LAST (section 6).
...
DRY_RUN=1 bash /home/liker2/projects/os-eco/haru/scripts/kura.sh
DRY_RUN=1 bash /home/liker2/projects/os-eco/haru/scripts/suji.sh
DRY_RUN=1 bash /home/liker2/projects/os-eco/haru/scripts/tane.sh

# 3a. kura → kura (no upstream remote)
...
cd /home/liker2/projects/os-eco/kura && git checkout main && git merge rebrand-to-kura && git push origin main
gh repo rename kura

# 3b. suji → suji (D17: script's internal guard ensures push to origin only — never to jayminwest upstream)
cd /home/liker2/projects/os-eco/haru
bash scripts/suji.sh
cd /home/liker2/projects/os-eco/suji && git checkout main && git merge rebrand-to-suji && git push origin main   # NEVER upstream
gh repo rename suji

# 3c. tane → tane
cd /home/liker2/projects/os-eco/haru
bash scripts/tane.sh
cd /home/liker2/projects/os-eco/tane && git checkout main && git merge rebrand-to-tane && git push origin main
gh repo rename tane

# Haru rebrand was applied in-place during the mission; final disk + gh rename happens in section 6.
```
...
```bash
mv /home/liker2/projects/os-eco/kura  /home/liker2/projects/os-eco/kura
mv /home/liker2/projects/os-eco/suji  /home/liker2/projects/os-eco/suji
mv /home/liker2/projects/os-eco/tane /home/liker2/projects/os-eco/tane
```

---

## 6. Final haru disk + GitHub rename

This MUST run BEFORE any subsequent `cd /haru` reference. The operator must NOT have any active session inside `/haru` at this point.
...
mv /home/liker2/projects/os-eco/haru /home/liker2/projects/os-eco/haru
cd /home/liker2/projects/os-eco/haru
gh repo rename haru
```
...
for d in kura suji tane haru; do
  echo "=== $d ==="
  cd /home/liker2/projects/os-eco/$d
  # No old-token straggler refs (D21 PCRE lookbehind for .haru carve-out)
  if grep -rPi --exclude-dir=.git --exclude-dir=.haru --exclude-dir=node_modules \
       "(?<!\.)\b(haru|kura|suji|tane)\b" --include="*.ts" . | head -5; then
    echo "[FAIL] $d has old-token stragglers"; SAW_STRAGGLER=1
  fi
  # No old short-CLI refs (allow CHANGELOG + migrate-from-beads literal)
  if grep -rE --exclude-dir=.git --exclude-dir=.haru --exclude-dir=node_modules \
...
- `src/kura/client.test.ts` — `which ku` absent until `@hana/kura-cli` published
- `src/tane/client.test.ts` — `which ta` absent until `@hana/tane-cli` published
...
gh issue close 189 --repo liker0704/haru \
  --comment "Mission rebrand-hana-v2 complete. Renamed haru→haru, kura→kura, suji→suji, tane→tane. See haru/.overstory/missions/mission-1778354709476-rebrand-hana-v2 for artifacts."
```
...
- `.overstory/hooks.json` regen failure: run `ha init` (legacy `ov` if still installed) or hand-edit `OVERSTORY_*` → `HARU_*`.
...
- `package.json` `dependencies."@hana/kura-cli"` STAYS (D8) — transitive npm dep, not republished.
- `suji` repo has `upstream → jayminwest/suji.git`. Push to `origin` only (D17).

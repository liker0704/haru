...
// Use the canonical haru repo as a real test fixture
const HARU_ROOT = "/home/liker2/projects/os-eco/haru";

describe("analyzeProject", () => {
	test("returns valid ProjectContext shape", async () => {
		const ctx = await analyzeProject(HARU_ROOT);
	...
	test("completes in < 5s", async () => {
		...
		await analyzeProject(HARU_ROOT);
		expect(Date.now() - start).toBeLessThan(5000);
	}, 6000);

	test("structuralHash is deterministic for same inputs", async () => {
		const ctx1 = await analyzeProject(HARU_ROOT);
		const ctx2 = await analyzeProject(HARU_ROOT);

...
| Metric | Value |
| --- | --- |
| Production TypeScript files | 349 |
| Test TypeScript files | 251 |
| Production TypeScript LOC | 92,069 |
| Test TypeScript LOC | 102,547 |
| Command modules in `src/commands` | 58 |
| Mission modules in `src/missions` | 48 |
| Runtime adapters in `src/runtimes` | 14 |
| Base agent definitions in `agents/` | 32 |

Large production hotspots:

| File | Approx LOC | Role |
| --- | ---: | --- |
| `src/watchdog/daemon.ts` | 2,296 | tier-0 health loop, escalation, reconciliation, mission engine tick |
| `src/missions/store.ts` | 1,356 | mission records and state inside sessions.db |
| `src/commands/coordinator.ts` | 1,210 | persistent coordinator lifecycle |
...
| `src/mail/store.ts` | 1,063 | messaging store with migrations and DLQ |
...
| `src/sessions/store.ts` | 997 | session lifecycle store |
| `src/worktree/tmux.ts` | 960 | tmux session control and headless process management |

High fan-in shared modules:

| Module | Imported by approx files | Meaning |
| --- | ---: | --- |
| `src/types.ts` | 133 | barrel re-export of 32 domain type files (see `src/*/types.ts`) |
...
Note: `src/types.ts` was decomposed from a monolithic shared-kernel schema into a barrel re-export. Actual type definitions now live in 32 domain-specific type files (`src/agents/types.ts`, `src/missions/types.ts`, `src/mail/types.ts`, etc.). New code should import directly from the domain type files.
...
- **Layer 1 (Base):** Reusable `.md` files in `agents/` define capabilities and workflows. 32 base definitions cover builders, reviewers, coordinators, scouts, leads, and other roles.
- **Layer 2 (Overlay):** Per-task `CLAUDE.md` generated by `ha sling` and written to the agent's worktree. Defines task-specific context: task ID, file scope, spec path, branch name, and parent agent.
...
| Profiler | `src/profiler/*` | builds `ProfilerTrace` from event/session/metrics stores; webserver pages at `src/webserver/pages/profiler*.ts` |
| Session insights | `src/insights/*` | session insight analyzer; reads `EventStore` from completed sessions, extracts tool-usage, file-edit, and error patterns |
...
| `cursor` | experimental | `.cursor/rules/haru.md` | interactive tmux |

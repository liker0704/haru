...
- **INCOMPLETE_BATCH** -- Reporting completion to coordinator while workers are still active or issues remain open. Verify via `ha group status` and `{{TRACKER_CLI}} show` for all issues before closing.
...
7. **Dispatch workers** for parallel work streams:
   ```bash
   ha sling --task <task-id> --capability builder --name <descriptive-name> \
...
- **Checkpoints** are saved to `.overstory/agents/$HARU_AGENT_NAME/checkpoint.json` before compaction or handoff. The checkpoint contains: agent name, assigned task ID, active worker IDs, task group ID, session ID, progress summary, and files modified.

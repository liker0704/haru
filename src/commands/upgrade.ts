...
async function executeUpgradeSingle(opts: UpgradeOptions): Promise<void> {
	...
	let latest: string;
	try {
		[current, latest] = await Promise.all([getCurrentVersion(), fetchLatestVersion(HARU_PACKAGE)]);
	} catch (err) {
	...
	if (!json) {
		process.stdout.write(`${muted(`Upgrading ${HARU_PACKAGE} from ${current} to ${latest}...`)}\n`);
	}
